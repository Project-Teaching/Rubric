<script lang="ts">
  import { auth, user } from "$lib/firebase";
  import { onMount } from 'svelte';
  import { writable } from 'svelte/store';
  import { goto } from '$app/navigation';
  import {
      GoogleAuthProvider,
      signInWithPopup,
      signOut,
  } from "firebase/auth";
  import { locale, t } from 'svelte-i18n'
  // @ts-ignore
  import MdLanguage from 'svelte-icons/md/MdLanguage.svelte'
  
  const theme = writable('LRtheme');
  let photoURL = '/user.png'; // Valor padrão para a imagem

  $: if ($user) {
    photoURL = $user.photoURL ? `${$user.photoURL}?t=${new Date().getTime()}` : '/user.png';
  } else {
    photoURL = '/user.png';
  }
  
  async function signInWithGoogle() {
      const provider = new GoogleAuthProvider();
      const credential = await signInWithPopup(auth, provider);
      const idToken = await credential.user.getIdToken();
      const res = await fetch("/api/signin", {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
          },
          body: JSON.stringify({ idToken }),
      });
  }
  
  async function signOutSSR() {
      const res = await fetch("/api/signin", { method: "DELETE" });
      await signOut(auth);
      goto("/");
  }

  let isDarkMode = false;
  let logoURL = '/RProLogoWhite.png';
  let logoWhite = '/RProLogoWhite.png';
  let logoDark = '/RProLogoDark.png';
  let logoClass = 'logo-visible';

  function toggleTheme() {
    logoClass = 'logo-hidden'; // fade start
    setTimeout(() => {
      const newTheme = isDarkMode ? 'light' : 'dark';
      // Atualize o estado isDarkMode antes de alterar a classe
      isDarkMode = !isDarkMode;
      theme.set(newTheme);
      localStorage.setItem('theme', newTheme);
      
      // Remove a classe do tema atual e adiciona a nova classe
      document.documentElement.classList.remove(isDarkMode ? 'light' : 'dark');
      document.documentElement.classList.add(newTheme);

      // Mude o logo conforme o novo tema
      logoURL = isDarkMode ? logoWhite : logoDark;

      logoClass = 'logo-visible'; // fade end
    }, 150); // atraso
  }

  function changeLanguage(lang: string) {
    locale.set(lang);
  }

  onMount(() => {
    const savedTheme = localStorage.getItem('theme') || 'light';
    theme.set(savedTheme);
    document.documentElement.classList.add(savedTheme);
    isDarkMode = savedTheme === 'dark';
    logoURL = isDarkMode ? logoWhite: logoDark;
  });
</script>

<div class="navbar bg-secondary-500 dark:bg-dark-secondary main-navbar-h shadow-sm">
  <div class="flex-1">
    <label for="my-drawer" class="btn btn-ghost text-xl drawer-button">||| <div class="w-24 ml-2 h-10"><img src={logoURL} alt="Logo" class="logo-transition {logoClass}"></div></label>
  </div>
  <div class="flex-none gap-2">
    {#if $user}
      <div class="font-bold">
        {$user.displayName}
      </div>
      <div class="dropdown dropdown-end">
        <div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar">
          <div class="w-10 rounded-full">
            <img src={photoURL} alt="User Profile" on:error="{() => photoURL = '/user.png'}" />
          </div>
        </div>
        <ul class="menu menu-sm rounded-xl dropdown-content bg-secondary-500 dark:bg-dark-secondary rounded-box z-[1] mt-4 w-52 p-2 shadow">
          <li class="text-base font-medium">
            <a href="#" class="justify-between">
              {$t('account')}
            </a>
          </li>
          <li class="text-base font-medium"><button on:click={signOutSSR}>{$t('logout')}</button></li>
        </ul>
      </div>
    {:else}
      <button on:click={signInWithGoogle} class="btn btn-ghost">Login</button>
      <div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar">
        <div class="w-10 rounded-full">
          <img alt="User Profile" src="/user.png" />
        </div>
      </div>
    {/if}
    <label class="swap swap-rotate" id="theme-switcher">
      <input type="checkbox" class="theme-controller hidden" on:change={toggleTheme} checked={isDarkMode} />
      <svg class="swap-off h-10 w-10 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
      </svg>
      <svg class="swap-on h-10 w-10 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
      </svg>
    </label>
    <div class="dropdown dropdown-end" id="language-dropdown">
      <button class="btn btn-ghost h-14 w-14 m-0 p-2"><MdLanguage /></button>
      <ul role="menu" class="menu z-10 dropdown-content mt-2 p-2 shadow bg-secondary-500 dark:bg-dark-secondary rounded-box w-40">
        <li><button on:click={() => changeLanguage('en')}>{$t('english')}</button></li>
        <li><button on:click={() => changeLanguage('pt')}>{$t('portuguese')}</button></li>
        <li><button on:click={() => changeLanguage('es')}>{$t('spanish')}</button></li>
      </ul>
    </div>    
  </div>
</div>
