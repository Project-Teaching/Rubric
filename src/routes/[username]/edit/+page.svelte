...
<main class="max-w-xl mx-auto">
  {#if $userData?.username == $page.params.username}
    ...
    <div class="text-center my-4">
      <a class="btn btn-outline btn-xs" href="/login/photo">Editar Imagem</a>
      ...
      <label class="label cursor-pointer flex items-start justify-center">
        <span class="label-text mr-6">
          <div
            ...
            data-tip="Se público, qualquer pessoa pode ver seu perfil"
          >
            {$userData?.published ? "Perfi público" : "Perfil privado"}
          </div>
        </span>
        <input
          type="checkbox"
    ...
    {#if showForm}
      ...
      >
        ...
        <div class="my-4">
          {#if !titleIsValid}
            <p class="text-error text-xs">O título precisa ser válido</p>
          {/if}
          {#if !urlIsValid}
            <p class="text-error text-xs">A URL precisa ser válida</p>
          {/if}
          {#if formIsValid}
            <p class="text-success text-xs">Tudo certo!</p>
          {/if}
        </div>
        ...
        <button
          ...
          type="submit"
          class="btn btn-success block">Adicionar Link</button
        >
